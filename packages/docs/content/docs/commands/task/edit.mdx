...
Edit an existing task plan by selecting it from a list and sending it to the backend agent for refinement. The agent reads the current task content and applies improvements or requested changes.
