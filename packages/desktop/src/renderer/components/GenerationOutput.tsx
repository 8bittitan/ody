  ...
  return (
    <section className="bg-panel/92 border-edge flex h-full flex-col rounded-lg border p-4 backdrop-blur-sm">
      ...
      {streamOutput.trim().length > 0 ? (
        <pre
          className="bg-background border-edge flex-1 overflow-auto rounded border p-2 font-mono text-[11px] whitespace-pre-wrap text-zinc-200"
          dangerouslySetInnerHTML={{ __html: streamOutputHtml }}
        />
      ) : (
        <pre className="bg-background border-edge flex-1 overflow-auto rounded border p-2 font-mono text-[11px] whitespace-pre-wrap text-zinc-200">
